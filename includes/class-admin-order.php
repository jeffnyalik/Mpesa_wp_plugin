...
class Plug_One_Admin_Order {
	...
	public static function meta_boxes() {
		...
		foreach ( $screens as $screen ) {
			add_meta_box(
				'plug-one-mpesa',
				__( 'M-Pesa (Plug One)', 'plug-one-payment-gateway-m-pesa' ),
	...
	public static function render( $post_or_order ) {
		...
		if ( ! $order || PLUG_ONE_GATEWAY_ID !== $order->get_payment_method() ) {
			echo '<p>' . esc_html__( 'This order did not use Plug One M-Pesa.', 'plug-one-payment-gateway-m-pesa' ) . '</p>';
			return;
		}
		...
		echo '<p><strong>' . esc_html__( 'Status', 'plug-one-payment-gateway-m-pesa' ) . ':</strong> ' . esc_html( $status ? $status : '—' ) . '</p>';
		echo '<p><strong>' . esc_html__( 'Phone', 'plug-one-payment-gateway-m-pesa' ) . ':</strong> ' . esc_html( $phone ? $phone : '—' ) . '</p>';
		echo '<p><strong>' . esc_html__( 'Receipt', 'plug-one-payment-gateway-m-pesa' ) . ':</strong> ' . esc_html( $receipt ? $receipt : '—' ) . '</p>';
		echo '<p><strong>' . esc_html__( 'CheckoutRequestID', 'plug-one-payment-gateway-m-pesa' ) . ':</strong><br><code>' . esc_html( $chk ? $chk : '—' ) . '</code></p>';
		if ( $desc ) {
		...
		wp_send_json_success(
			array(
				'message' => sprintf(
					...
					__( 'Status: %s', 'plug-one-payment-gateway-m-pesa' ),
					$status
				),
			)
		);
	...
	public static function ajax_test() {
		...
		if ( ! class_exists( 'Plug_One_Daraja_Client' ) ) {
			wp_send_json_error( array( 'message' => __( 'Daraja client is not available in this installation.', 'plug-one-payment-gateway-m-pesa' ) ) );
		}
		...
		try {
			...
			wp_send_json_success( array( 'message' => __( 'Daraja OAuth succeeded.', 'plug-one-payment-gateway-m-pesa' ) ) );
		} catch ( Exception $e ) {
			wp_send_json_error( array( 'message' => $e->getMessage() ) );
		}
	}
	...
	public static function ajax_simulate() {
		self::guard();
		if ( ! self::can_simulate() ) {
			wp_send_json_error( array( 'message' => __( 'Simulate payment is only available in sandbox / WP_DEBUG.', 'plug-one-payment-gateway-m-pesa' ) ) );
		}
		$order = self::order_from_request();
		try {
			...
			wp_send_json_success(
				array(
					'message' => sprintf(
						...
						__( 'Simulated payment. Receipt: %s', 'plug-one-payment-gateway-m-pesa' ),
						$receipt
					),
				)
			);
	...
	protected static function guard() {
		...
		if ( ! current_user_can( 'manage_woocommerce' ) ) {
			wp_send_json_error( array( 'message' => __( 'Permission denied.', 'plug-one-payment-gateway-m-pesa' ) ), 403 );
		}
	}
	...
	protected static function order_from_request() {
		...
		if ( ! $order ) {
			wp_send_json_error( array( 'message' => __( 'Order not found.', 'plug-one-payment-gateway-m-pesa' ) ) );
		}
		return $order;
	}
}
