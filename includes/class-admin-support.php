...
class Plug_One_Admin_Support {
	...
	public static function action_links( $links ) {
		$extra = array(
			'<a href="' . esc_url( admin_url( 'admin.php?page=wc-settings&tab=checkout&section=' . PLUG_ONE_GATEWAY_ID ) ) . '">' . esc_html__( 'Settings', 'plug-one' ) . '</a>',
			'<a href="' . esc_url( admin_url( 'admin.php?page=plug-one-support' ) ) . '">' . esc_html__( 'Docs & support', 'plug-one' ) . '</a>',
		);
	...
	public static function render() {
		...
		echo '<h1>' . esc_html__( 'Plug One — Docs & support', 'plug-one' ) . '</h1>';

		echo '<div class="plug-one-admin-panel" style="max-width:720px;margin-top:1rem;">';
		echo '<h2>' . esc_html__( 'License', 'plug-one' ) . '</h2>';
		if ( $pro ) {
			echo '<p><span class="plug-one-ok">' . esc_html__( 'Pro features unlocked (STK Push).', 'plug-one' ) . '</span></p>';
		} else {
			echo '<p><span class="plug-one-err">' . esc_html__( 'Free mode: Manual Paybill/Till only. Activate Pro for STK Push.', 'plug-one' ) . '</span></p>';
		}
		if ( $fs_on && $pricing ) {
		...
		self::doc_link( __( 'Callbacks & ngrok', 'plug-one' ), $docs ? trailingslashit( $docs ) . 'callbacks/' : $local_docs . 'callbacks.md' );
		self::doc_link( __( 'Production Till checklist', 'plug-one' ), $docs ? trailingslashit( $docs ) . 'production-till/' : $local_docs . 'production-till.md' );
		self::doc_link( __( 'Licensing & updates', 'plug-one' ), $docs ? trailingslashit( $docs ) . 'licensing/' : $local_docs . 'licensing.md' );
		self::doc_link( __( 'Troubleshooting', 'plug-one' ), $docs ? trailingslashit( $docs ) . 'troubleshooting/' : $local_docs . 'troubleshooting.md' );
		echo '</ul>';
		echo '</div>';
